...
const _createFilesInFolder = (collectionName, directory, individualItem) => {
...
const _createSingleFile = (collectionName, singleTest) => {
  const json = JSON.stringify(singleTest, null, 2);
  const file = _.toLower(singleTest.name.split(' ').join('_'));
  ...
  return Promise.map(collection.item, item => {
    ...
    if (item.item) {
      ...
      return fs.mkdirAsync(`test/${collectionName}Collection/${directory}`)
        .then(() => {
          return _createFilesInFolder(collectionName, directory, item.item);
        })
        .catch(err => {
          if (err.code === 'EEXIST') {
            return _createFilesInFolder(collectionName, directory, item.item);
      ...
      return _createSingleFile(collectionName, item);
    }
  });
};
